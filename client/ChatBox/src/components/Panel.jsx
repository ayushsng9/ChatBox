...
>
  ...
  <div className='flex flex-col flex-grow min-w-0'>
    <h3 className='text-ellipsis whitespace-nowrap overflow-hidden font-semibold text-base'>
      {conv?.userDetails?.name}
    </h3>
    <div className='text-slate-500 text-xs flex items-center gap-1 min-w-0'>
      <div className='flex items-center gap-1 min-w-0 max-w-'>
        {conv?.lastMsg?.image && (
      ...
      <p className='text-ellipsis whitespace-nowrap overflow-hidden min-w-0 flex-grow' style={{maxWidth:"140px"}}>
        {conv?.lastMsg?.text}
      </p>
    </div>
  </div>
  {Boolean(conv?.unseenMsg) && (
    <p className='text-xs ml-auto w-6 h-6 flex justify-center items-center p-1 bg-sky-400 text-white font-semibold rounded-full'>
      {conv?.unseenMsg}
    </p>
  )}
</NavLink>
